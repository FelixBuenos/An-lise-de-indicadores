...
} from "./js/componentes/avaliacao.js?v=1003";
...
async function buscarDadosRH(nomeFilial, periodoSelecionado) {
	...
	try {
		...
		if (errFilial || !dadosFilial) {
			limparTabelaRH();
			console.warn(
				`Nenhum dado encontrado em dados_rh para a filial: ${nomeFilial}`,
			);
			return;
		}
		...
		document.getElementById("rh-folha").textContent = formatarPorcentagem(
			dadosFilial.folha_pagamento,
		);
		document.getElementById("rh-comissao").textContent = formatarPorcentagem(
			dadosFilial.comissao,
		);
		document.getElementById("rh-premiacao").textContent = formatarPorcentagem(
			dadosFilial.premiacao,
		);
		document.getElementById("rh-horas").textContent = formatarPorcentagem(
			dadosFilial.horas_extras,
		);
		document.getElementById("rh-perdas").textContent = formatarPorcentagem(
			dadosFilial.perdas,
		);
		...
		if (
			errCluster ||
			!dadosCluster ||
			dadosCluster.length === 0 ||
			!dadosCluster[0].cluster
		) {
			console.warn(
				"Cluster não encontrado para as datas informadas na tabela_completa.",
			);
			return;
		}
		...
		const { data: listaFiliais } = await supabase
			.from("tabela_completa")
			.select("filial")
		...
		const { data: dadosDoGrupo } = await supabase
			.from("dados_rh")
			.select("*")
		...
		document.getElementById("cluster-folha").textContent = formatarPorcentagem(
			calcularMedia(dadosDoGrupo, "folha_pagamento"),
		);
		document.getElementById("cluster-comissao").textContent =
			formatarPorcentagem(calcularMedia(dadosDoGrupo, "comissao"));
		document.getElementById("cluster-premiacao").textContent =
			formatarPorcentagem(calcularMedia(dadosDoGrupo, "premiacao"));
		document.getElementById("cluster-horas").textContent = formatarPorcentagem(
			calcularMedia(dadosDoGrupo, "horas_extras"),
		);
		document.getElementById("cluster-perdas").textContent = formatarPorcentagem(
			calcularMedia(dadosDoGrupo, "perdas"),
		);
	} catch (err) {
