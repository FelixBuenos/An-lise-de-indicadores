...
export async function obterDadosGraficos(periodoSelecionado, periodoAnterior) {
    ...
    const dataFimAnterior = periodoAnterior ? periodoAnterior.split('|')[1] : null;

    try {
        const [resBaixo, resCritico, resAtualCompleto, resAnteriorCompleto] = await Promise.all([
            supabase
        ...
        const topOscilacao = listaOscilacao.sort((a, b) => b.valor - a.valor).slice(0, 10);
