...
export async function inicializarAppLauncher() {
    ...
    document.addEventListener('click', (e) => {
        if (!menuAppLauncher.contains(e.target) && !btnAppLauncher.contains(e.target)) {
            menuAppLauncher.classList.add('hidden');
            btnAppLauncher.classList.remove('ativo');
        }
    });
    ...
    try {
        ...
        // Renderiza cada item
        gridContainer.innerHTML = solucoes.map(item => {
            const urlDestino = item.url && item.url.trim() !== '' && item.url !== '#' ? item.url : '#';
            const targetAttr = urlDestino !== '#' ? 'target="_blank" rel="noopener noreferrer"' : '';
            
            let iconeHtml = '';
            const iconeStr = (item.icone || '').trim();

            if (iconeStr.startsWith('http://') || iconeStr.startsWith('https://') || iconeStr.startsWith('/') || iconeStr.startsWith('imagens/') || iconeStr.endsWith('.png') || iconeStr.endsWith('.svg') || iconeStr.endsWith('.jpg') || iconeStr.endsWith('.jpeg')) {
