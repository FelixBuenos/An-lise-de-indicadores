...
async function checarPerfilGestorNoBanco(emailUsuario) {
    ...
    try {
        ...
        if (error) {
            console.warn('Tabela de gestores não encontrada ou erro na consulta:', error.message);
            return false;
        }
...
async function iniciarSistema() {
    ...
    analistaLogado = session.user.email || session.user.user_metadata?.name || null;
    ...
    if (badgePerfil) {
        ...
        if (ehGestorLogado) {
            ...
            badgePerfil.innerHTML = '<i class="fas fa-user-shield"></i> Administrador Geral (Acesso Total)';
            badgePerfil.title = 'Você possui permissão de Administrador Geral para editar, concluir e excluir qualquer demanda.';
        } else {
...
function atualizarIconeOrdenacao() {
    ...
    if (ordemDataAsc) {
        ...
        thData.title = 'Ordenado do mais antigo para o mais novo. Clique para ver do mais novo para o mais antigo.';
    } else {
        icone.className = 'fas fa-sort-numeric-down-alt';
        thData.title = 'Ordenado do mais novo para o mais antigo. Clique para ver do mais antigo para o mais novo.';
    }
}
...
async function carregarFiltroLojas() {
    ...
    if (dadosLojas) dadosLojas.forEach(d => d.filial && todasLojas.add(d.filial.trim()));
    if (dadosDemandas) dadosDemandas.forEach(d => d.loja && todasLojas.add(d.loja.trim()));
...
async function carregarFiltroCriadores() {
    ...
    Array.from(criadoresUnicos).sort().forEach(criador => {
        const option = document.createElement('option');
...
async function carregarDemandas() {
    ...
    const filtroFerramenta = document.getElementById('filtro-ferramenta')?.value || '';
...
function aplicarBuscaRapida() {
    const termoBusca = (document.getElementById('campo-busca-rapida')?.value || '').toLowerCase().trim();
    ...
    if (!termoBusca) {
        ...
        demandasFiltradas = todasDemandasDoBanco.filter(d => {
            ...
            return loja.includes(termoBusca) ||
...
function renderizarTabela(data) {
    ...
    // Renderizar tabela
    tbody.innerHTML = data.map((demanda, index) => {
        ...
        // Badge de prazo / SLA
        let prazoHtml = '<span style="color: #94a3b8; font-size: 0.8rem;">-</span>';
        if (demanda.prazo) {
        ...
        // Badge de status
        const classeBadgeStatus = ehConcluida ? 'badge-status-concluido' : 'badge-status-pendente';
        const iconeStatus = ehConcluida ? '<i class="fas fa-check-circle"></i>' : '<i class="fas fa-clock"></i>';
        ...
        if (ehConcluida) {
            const dataConclusao = demanda.data_conclusao ? formatarDataBr(demanda.data_conclusao) : '';
            if (podeAlterar) {
                acaoStatusHtml = `
                    <span class="demanda-concluido-texto" title="Concluído em ${dataConclusao}">
                        <i class="fas fa-check-circle"></i> ${dataConclusao || 'Concluído'}
                    </span>
                    <button class="btn-reabrir-demanda" data-id="${demanda.id}" title="Reabrir demanda para pendente">
            ...
            if (podeAlterar) {
                acaoStatusHtml = `
                    <button class="btn-concluir-demanda" data-id="${demanda.id}" title="Marcar como concluída">
        ...
        if (podeAlterar) {
            botoesGestaoHtml = `
                <button class="btn-editar-demanda" data-id="${demanda.id}" title="Editar esta demanda">
                    <i class="fas fa-edit"></i> Editar
                </button>
                <button class="btn-excluir-demanda" data-id="${demanda.id}" title="Excluir esta demanda">
        ...
        return `
            <tr class="${classeRow}" style="animation-delay: ${index * 0.03}s">
                ...
                <td class="demanda-col-descricao">${demanda.descricao || '-'}</td>
    ...
    tbody.querySelectorAll('.btn-concluir-demanda:not([disabled])').forEach(btn => {
...
function exportarParaExcel() {
    ...
    const dadosPlanilha = demandasFiltradas.map(d => ({
        ...
        'Data de Conclusão': d.data_conclusao ? formatarDataBr(d.data_conclusao) : '-'
    }));
...
function abrirModalCadastro() {
    ...
    if (titulo) titulo.innerHTML = '<i class="fas fa-plus-circle" style="margin-right: 8px;"></i> Cadastrar Nova Demanda';
...
function abrirModalEdicao(idDemanda) {
    const demanda = todasDemandasDoBanco.find(d => String(d.id) === String(idDemanda));
    if (!demanda) {
    ...
    if (!verificarPermissao(demanda.analista)) {
        alert('Apenas o autor desta demanda ou o Administrador Geral podem editá-la.');
        return;
    }
    ...
    if (titulo) titulo.innerHTML = '<i class="fas fa-edit" style="margin-right: 8px;"></i> Editar Demanda';
    if (btnTexto) btnTexto.textContent = 'Atualizar Demanda';

    if (selectLoja) selectLoja.value = demanda.loja || '';
    if (selectFerramenta) selectFerramenta.value = demanda.ferramenta || '';
    if (selectPrioridade) selectPrioridade.value = demanda.prioridade || 'Média';
    if (selectResp) selectResp.value = demanda.responsavel || '';
    if (inputData) inputData.value = demanda.data_criacao || new Date().toISOString().split('T')[0];
...
async function salvarDemanda() {
    ...
    const ferramenta = selectFerramenta ? selectFerramenta.value.trim() : '';
    const prioridade = selectPrioridade ? selectPrioridade.value.trim() : 'Média';
    const responsavel = selectResp ? selectResp.value.trim() : '';
    const dataCriacao = inputData ? inputData.value : new Date().toISOString().split('T')[0];
    const prazo = inputPrazo && inputPrazo.value ? inputPrazo.value : null;
    ...
    if (idDemanda) {
        ...
        const demandaExistente = todasDemandasDoBanco.find(d => String(d.id) === String(idDemanda));
        if (demandaExistente && !verificarPermissao(demandaExistente.analista)) {
            alert('Apenas o autor desta demanda ou o Administrador Geral podem atualizá-la.');
            return;
        }
...
async function excluirDemanda(idDemanda, botao) {
    ...
    if (demanda && !verificarPermissao(demanda.analista)) {
        alert('Apenas o autor desta demanda ou o Administrador Geral podem excluí-la.');
        return;
    }

    if (!confirm('Tem certeza de que deseja EXCLUIR esta demanda definitivamente? Esta ação não pode ser desfeita.')) {
        return;
    }
...
async function alterarStatusDemanda(idDemanda, novoStatus, botao) {
    const demanda = todasDemandasDoBanco.find(d => String(d.id) === String(idDemanda));
    if (demanda && !verificarPermissao(demanda.analista)) {
        alert('Apenas o autor desta demanda ou o Administrador Geral podem alterar o status desta demanda.');
        return;
    }

    const msgConfirm = novoStatus === 'concluido'
...
function verificarPermissao(analistaCriador) {
    ...
    if (logadoLimpo.length >= 4 && criadorLimpo.includes(logadoLimpo)) return true;
    if (criadorLimpo.length >= 4 && logadoLimpo.includes(criadorLimpo)) return true;
...
function animarContador(elemento, valorFinal) {
    ...
    function atualizar(tempoAtual) {
        ...
        const valor = Math.round(valorAtual + (valorFinal - valorAtual) * progresso);
...
function configurarEventos() {
    ...
    if (selectLoja) selectLoja.addEventListener('change', () => carregarDemandas());
    if (selectFerramenta) selectFerramenta.addEventListener('change', () => carregarDemandas());
    if (selectPrioridade) selectPrioridade.addEventListener('change', () => carregarDemandas());
    if (selectResp) selectResp.addEventListener('change', () => carregarDemandas());
    if (selectCriador) selectCriador.addEventListener('change', () => carregarDemandas());
    if (selectStatus) selectStatus.addEventListener('change', () => carregarDemandas());
