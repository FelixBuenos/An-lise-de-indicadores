 ...
 */
export async function renderizarDiagnostico(supabase, filialEscolhida, periodoEscolhido) {
...
async function processarAnaliseComparativa() {
    ...
    let comparandoComMeta = (tipoComparacao === 'metas_mercado');
    ...
    if (tipoComparacao === 'semana_2') {
        ...
        const emMemoria = registrosSemanasAtuais.find(r => r.data_final === dataFim);
        if (emMemoria) {
    ...
    gerarRelatorioInsights(regAtual, regComparacao, comparandoComMeta);
}

/**
 * Analisa todos os indicadores e gera os cards detalhados de Pontos Fortes e Pontos de Atenção
 */
function gerarRelatorioInsights(atual, comp, apenasMetas) {
    ...
    const badgePositivos = document.getElementById('contador-diagnostico-positivo');
    ...
    const METAS = {
        ...
        desconto: 0.30,        // <= 30%
        falta: 0.09,           // <= 9%
        aproveitamento: 0.60,  // >= 60%
        ticket_medio: 50.00,   // >= R$ 50,00
        itens_cliente: 2.50,   // >= 2,5 itens
    ...
    if (ticketAtual > 0) {
        ...
        const deltaTicket = ticketComp > 0 ? ((ticketAtual - ticketComp) / ticketComp) * 100 : 0;
    ...
    if (badgePositivos) badgePositivos.innerHTML = `<i class="fas fa-check-circle"></i> ${pontosFortes.length} Pontos Fortes`;
    if (badgeAtencao) badgeAtencao.innerHTML = `<i class="fas fa-exclamation-triangle"></i> ${pontosAtencao.length} Pontos de Atenção`;

    // Renderiza HTML das 2 listas
    listaFortes.innerHTML = pontosFortes.map(p => `
        <li class="item-insight">
            <strong>${p.titulo} ${p.delta ? `<span class="tag-delta tag-delta-${p.tipoDelta}">${p.delta}</span>` : ''}</strong>
            ${p.texto}
        </li>
    `).join('') || '<li class="item-insight">Nenhum ponto forte identificado no comparativo.</li>';

    listaAtencao.innerHTML = pontosAtencao.map(p => `
        <li class="item-insight">
            <strong>${p.titulo} ${p.delta ? `<span class="tag-delta tag-delta-${p.tipoDelta}">${p.delta}</span>` : ''}</strong>
            ${p.texto}
        </li>
    `).join('') || '<li class="item-insight">Nenhum desvio crítico identificado na semana.</li>';
}
