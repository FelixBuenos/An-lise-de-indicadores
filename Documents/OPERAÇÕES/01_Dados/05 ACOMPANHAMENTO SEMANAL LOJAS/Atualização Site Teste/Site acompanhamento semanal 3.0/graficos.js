...
function destruirGraficos() {
	if (chartPositivo) {
		chartPositivo.destroy();
		chartPositivo = null;
	}
	if (chartCritico) {
		chartCritico.destroy();
		chartCritico = null;
	}
	if (chartOscilacao) {
		chartOscilacao.destroy();
		chartOscilacao = null;
	}
}

function alternarVisibilidadeMensagem(
	elementIdCanvas,
	elementIdMsg,
	possuiDados,
) {
	const canvas =
		document.getElementById(elementIdCanvas) ||
		document.getElementById(
			elementIdCanvas.replace(/([A-Z])/g, "-$1").toLowerCase(),
		);
...
function renderizarGraficos(dados) {
	...
	const canvasPositivo =
		document.getElementById("chartPositivo") ||
		document.getElementById("chart-positivo");
	const canvasCritico =
		document.getElementById("chartCritico") ||
		document.getElementById("chart-critico");
	const canvasOscilacao =
		document.getElementById("chartOscilacao") ||
		document.getElementById("chart-oscilacao");
	...
	alternarVisibilidadeMensagem(
		"chartPositivo",
		"msg-sem-dados-positivo",
		possuiPositivos,
	);
	alternarVisibilidadeMensagem(
		"chartCritico",
		"msg-sem-dados-critico",
		possuiCriticos,
	);
	alternarVisibilidadeMensagem(
		"chartOscilacao",
		"msg-sem-dados-oscilacao",
		possuiOscilacao,
	);
	...
	if (dados.distribuicao) {
		...
		if (countBaixo)
			countBaixo.textContent = `(${dados.distribuicao.saudavel.total})`;
		if (countMedio)
			countMedio.textContent = `(${dados.distribuicao.alerta.total})`;
		if (countAlto)
			countAlto.textContent = `(${dados.distribuicao.critico.total})`;
		...
		if (btnBaixo) {
			...
			document
				.getElementById("btn-status-baixo")
				.addEventListener("click", () => {
					exibirTabelaDetalhamento(
						"Baixo",
						dados.distribuicao.saudavel.lojas,
						"#10b981",
					);
				});
		}
		if (btnMedio) {
			...
			document
				.getElementById("btn-status-medio")
				.addEventListener("click", () => {
					exibirTabelaDetalhamento(
						"Médio",
						dados.distribuicao.alerta.lojas,
						"#f59e0b",
					);
				});
		}
		if (btnAlto) {
			...
			document
				.getElementById("btn-status-alto")
				.addEventListener("click", () => {
					exibirTabelaDetalhamento(
						"Alto",
						dados.distribuicao.critico.lojas,
						"#ef4444",
					);
				});
		}
