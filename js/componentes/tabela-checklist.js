...
export async function preencherTabelaChecklist(supabase, filialEscolhida, periodoEscolhido) {
    ...
    let { data: dataLojaAtual, error: errorLojaAtual } = await supabase
    ...
    let { data: dataLojaAnterior, error: errorLojaAnterior } = await supabase
        .from('nota_checklist_lojas')
        .select('area, nota')
        .eq('filial', filialEscolhida)
    ...
    let { data: dataRede, error: errorRede } = await supabase
    ...
    linhasTabela.forEach(linha => {
        ...
        if (celulaArea) {
            ...
            if (dicLojaAnterior[nomeAreaBusca] !== undefined) {
                const notaMesAnteriorNum = Math.round(dicLojaAnterior[nomeAreaBusca] * 100);
    ...
    try {
        const { data: dataEncarteAnterior, error: errEncarte } = await supabase
        ...
        const tdMesAnteriorEncarte = document.getElementById('mes-anterior-checklist-encarte');
        ...
        console.warn("Nenhum dado de encarte anterior encontrado para a filial:", filialEscolhida);
    }
}