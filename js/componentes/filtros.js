...
export async function inicializarFiltros(supabase) {
	...
	const urlFilialParam = urlParams.get("filial");
	...
	function encontrarIndiceLoja(select, termo) {
		...
		if (termoSemConviva) {
			for (let i = 0; i < select.options.length; i++) {
				const val = (select.options[i].value || "").trim().toLowerCase();
				const txt = (select.options[i].text || "").trim().toLowerCase();
				if (val.includes(termoSemConviva) || txt.includes(termoSemConviva))
					return i;
			}
		}
		...
		data.forEach((linha) => {
			...
			if (linha.data_final) {
				...
				if (!semanasPorMes[chaveMes].some((s) => s.chave === valorEscondido)) {
					semanasPorMes[chaveMes].push({
						chave: valorEscondido,
						texto: textoVisivel,
					});
				}
			}
	...
	async function atualizarFiltroFiliais(filialPreSelecionar = null) {
		...
		if (statusEscolhido) {
			filiaisFiltradas = filiaisFiltradas.filter((linha) => {
				...
				if (statusEscolhido === "critico")
					return s.includes("crítico") || s.includes("critico");
				if (statusEscolhido === "medio")
					return s.includes("médio") || s.includes("medio");
				if (statusEscolhido === "baixo") return s.includes("baixo");
				return true;
			});
		}

		let filiaisUnicasFiltradas = Array.from(
			new Set(filiaisFiltradas.map((f) => f.filial)),
		);

		if (redeEscolhida) {
			filiaisUnicasFiltradas = filiaisUnicasFiltradas.filter(
				(filial) => filialParaRede[filial] === redeEscolhida,
			);
		}
		...
		if (alvoFilial) {
			const jaTem = filiaisUnicasFiltradas.some(
				(f) =>
					f.toLowerCase().includes(alvoFilial.toLowerCase()) ||
					alvoFilial.toLowerCase().includes(f.toLowerCase()),
			);
			if (!jaTem) {
				const filialOriginal = todasFiliais.find(
					(f) =>
						f.toLowerCase().includes(alvoFilial.toLowerCase()) ||
						alvoFilial.toLowerCase().includes(f.toLowerCase()),
				);
	...
	if (periodoSelect)
		periodoSelect.addEventListener("change", () => atualizarFiltroFiliais());
	if (statusSelect)
		statusSelect.addEventListener("change", () => atualizarFiltroFiliais());
	if (redeSelect)
		redeSelect.addEventListener("change", () => atualizarFiltroFiliais());
	...
	async function carregarDadosChecklistEncarte(supabase, lojaSelecionada) {
		...
		try {
			...
			const notesValidas = notas
				.map((n) => parseFloat(n.resultado))
