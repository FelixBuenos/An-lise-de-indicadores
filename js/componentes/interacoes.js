...
export function inicializarDestaqueInterativo() {
	const linhasTabelaAzul = document.querySelectorAll(
		".tabela-principal tbody tr",
	);
	...
	linhasTabelaAzul.forEach((linha) => {
		linha.addEventListener("click", () => {
			...
			const thAlvo = tabelaPreta.querySelector(
				`thead tr th:nth-child(${colunaCssIndex})`,
			);

			// 2. VERIFICAÇÃO INTELIGENTE: A coluna alvo JÁ está destacada?
			const jaEstaDestacada = thAlvo?.classList.contains("destaque-coluna");
