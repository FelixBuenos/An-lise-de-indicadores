...
export async function preencherTabelaMedia(supabase, filialEscolhida, periodoEscolhido) {
    ...
    const clusterIdentificado = String(dadosLoja?.cluster || dadosLoja?.status || '').trim();
    const clusterNormalizado = clusterIdentificado.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    
    let termoBuscaCluster = "classe 2"; 
    if (clusterNormalizado.includes('classe 1') || clusterNormalizado === 'alto' || clusterNormalizado === 'alta') {
        termoBuscaCluster = 'classe 1';
    } else if (clusterNormalizado.includes('classe 2') || clusterNormalizado === 'medio' || clusterNormalizado === 'media') {
        termoBuscaCluster = 'classe 2';
    } else if (clusterNormalizado.includes('classe 3') || clusterNormalizado === 'baixo' || clusterNormalizado === 'baixa') {
        termoBuscaCluster = 'classe 3';
    } else if (clusterNormalizado.includes('classe 4')) {
        termoBuscaCluster = 'classe 4';
    } else if (clusterNormalizado.includes('classe 5') || clusterNormalizado === 'critico') {
    ...
    if (metasCluster && metasCluster.length > 0) {
        metasCluster.forEach(meta => {
            ...
            if (colunaIndex) {
                ...
                else if (indicadorNome === 'qtd cliente') tipoFormato = 'inteiro_formatado';
...
async function preencherLinhaMediaClasseComFerramenta(supabase, dadosClasse, filialEscolhida, dataFinalAtualStr) {
    ...
    try {
        ...
        const clusterAlvoNorm = clusterAlvo.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
        const statusAlvo = String(dadosClasse.status || '').trim();
        const statusAlvoNorm = statusAlvo.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");

        // 1. Busca todas as filiais da mesma semana para obter o conjunto de lojas da mesma classe
        let { data: registrosClasse, error: errClasse } = await supabase
            .from('tabela_completa')
        ...
        if (registrosClasse && registrosClasse.length > 0) {
            ...
            const lojasMesmaClasse = registrosClasse.filter(r => {
                if (clusterAlvoNorm) {
                    const c = String(r.cluster || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
                    if (c) return c === clusterAlvoNorm;
                }
                if (statusAlvoNorm) {
                    const s = String(r.status || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
            ...
            if (qtdsDiretas.length > 0) {
                ...
                const nomesFiliais = listaLojasUnicas.map(r => r.filial).filter(Boolean);

                if (nomesFiliais.length > 0) {
                    const { data: dadosSemana, error: errSemana } = await supabase
                        .from('dados_lojas')
        ...
        console.error('Erro ao calcular média de Qtd Cliente da classe:', errCalculo);
    }
    ...
    celulas[3].textContent = formatarValor(dadosClasse.media_itens_por_cliente, 'decimal');
    celulas[4].textContent = formatarValor(dadosClasse.media_aproveitamento, 'percentual');
    celulas[5].textContent = formatarValor(dadosClasse.media_ticket_medio, 'moeda');
    celulas[6].textContent = formatarValor(dadosClasse.media_cmv, 'percentual');
    celulas[8].textContent = formatarValor(dadosClasse.media_cobertura, 'inteiro_puro');
    celulas[9].textContent = formatarValor(dadosClasse.media_excesso, 'inteiro_puro');
    celulas[10].textContent = formatarValor(dadosClasse.media_falta, 'percentual');
            ...
            if (ferramentaRaw) {
                ...
                if (nomesLojasMesmaFerramenta.length > 0) {
                    ...
                    if (dadosClasse.cluster) {
                        queryClasseFerramenta = queryClasseFerramenta.eq('cluster', dadosClasse.cluster);
                    } else if (dadosClasse.status) {
                        queryClasseFerramenta = queryClasseFerramenta.eq('status', dadosClasse.status);
                    }

                    const { data: registrosClasseFerramenta, error: errRegistros } = await queryClasseFerramenta;
