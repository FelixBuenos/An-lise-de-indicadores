...
export function abrirModalDemanda() {
    ...
    let nomeLojaAtual = selectLojaAtual && selectLojaAtual.selectedIndex >= 0
        ? selectLojaAtual.options[selectLojaAtual.selectedIndex].text
        : '';

    if (nomeLojaAtual.toLowerCase().includes('carregando') || nomeLojaAtual.toLowerCase().includes('selecione')) {
        nomeLojaAtual = '';
    }
...
async function salvarDemanda() {
    ...
    const dataCriacao = inputData && inputData.value ? inputData.value : new Date().toISOString().split('T')[0];
    ...
    if (!loja || loja.toLowerCase().includes('selecione') || loja.toLowerCase().includes('carregando')) {
        alert('Por favor, selecione uma filial no filtro do topo antes de cadastrar a demanda.');
        return;
    }
    ...
    if (!responsavel) {
        alert('Por favor, selecione quem é o responsável (Operações ou Associado).');
    ...
    alert('Demanda cadastrada com sucesso! Ela já está disponível no Gerenciador de Demandas.');
...
// ==========================================
export function inicializarModalAvaliacao(instanciaSupabase) {
    obterSessao();
...
export async function carregarHistorico(supabaseInst, filial) {
    ...
    let nomeLojaTexto = filial || (selectLoja && selectLoja.value ? selectLoja.value : '');

    if (selectLoja && selectLoja.selectedIndex >= 0) {
        const txt = selectLoja.options[selectLoja.selectedIndex].text;
        if (txt && !txt.toLowerCase().includes('carregando') && !txt.toLowerCase().includes('selecione')) {
            nomeLojaTexto = txt;
        }
    }

    if (!nomeLojaTexto || nomeLojaTexto.toLowerCase().includes('selecione') || nomeLojaTexto.toLowerCase().includes('carregando')) {
        tbody.innerHTML = `
            <tr>
    ...
    tbody.innerHTML = data.map((item) => {
        ...
        let iconePrioridade = '';
        if (prioridade === 'Alta') { corPrioridade = '#dc2626'; iconePrioridade = ''; }
        if (prioridade === 'Baixa') { corPrioridade = '#16a34a'; iconePrioridade = ''; }
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => inicializarModalAvaliacao(supabase));
} else {
